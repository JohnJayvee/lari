...
<form  method="POST" action="{{ route('permissions.update') }}" >
  @csrf
  <input type="hidden" id="id" name="id" value="{{ $permissions->planid }}"/>
    <div class="modal-header ">
      <h5 class="modal-title" id="modalTopTitle"><i class="bx bx-pencil me-1"></i> Permissions </h5>
    </div>
      ...
      <div class="row">
        ...
        <div class="col-6 mb-3">
          ...
          <input
            ...
            class="form-control datepicker"
            placeholder="Price"
            value="{{  number_format( $permissions->planprice , 2, '.', '')  }}"
            required
            autocomplete="off"
          />
        </div>

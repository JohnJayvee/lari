  ...
  <form  method="POST" action="{{ route('user.update') }}" >
    @csrf
    <input type="hidden" id="id" name="id" value="{{ $users->userid }}"/>
      <div class="modal-header ">
        <h5 class="modal-title" id="modalTopTitle"><i class="bx bx-pencil me-1"></i>Users</h5>
      </div>
        ...
        <div class="row">
          ...
          <div class="col-6 mb-3">
            <div class="form-password-toggle">
              <label class="form-label" for="unhashedpassword">Password</label>
              <div class="input-group">
                <input type="password" class="form-control password" name="password" id="password" placeholder="••••••••••" aria-describedby="password" autocomplete="off" value="{{$users->unhashedpassword}}"  required>
          ...
          <div class="col-6">
            ...
            <select class="form-select" name="userlevel" id="userlevel">
              @foreach($level as $list)
                <option value="{{$list->userlevel}}" {{$list->userlevel == $users->userlevel? 'selected' : '' }} >{{$list->uleveldescription}}</option>
              @endforeach
            </select>
          </div>
